  ...
  <div class="container">
    ...
    <section class="menu">
		...
		// Options
			...
			$vault = $_POST["vault"];
			$skipAtf = $_POST["skip-atf"];
			$skipAvs = $_POST["skip-avs"];
			$skipCvv = $_POST["skip-avv"];
    ...
    ?>
    	...
    	<?php if ($result->success): ?>
      	<h3>Success!</h3>
      	<p>Transaction ID is <?php echo($result->transaction->id)?>. Amount <?php echo($result->transaction->amount)?> has been <?php echo($result->transaction->status)?> with a service fee of <?php echo($result->transaction->serviceFeeAmount)?>.</p>
